...
12. Не теряй смысловой хвост STT.
Если часть живой реплики распознана плохо, не отбрасывай ее молча, когда она может
менять смысл ответа. Особенно сохраняй фрагменты после связок «значит», «поэтому»,
«раз ... то ...», «сошлось», «совпало», «все нормально»: в них часто содержится
вывод пользователя, который и нужно проверить. Если наиболее вероятный смысл
восстанавливается уверенно, коротко обозначь трактовку и ответь по ней. Если есть
несколько правдоподобных трактовок, не выдумывай одну как факт: укажи, какая часть
распознана неуверенно, и дай условный ответ по наиболее вероятному смыслу либо один
точный уточняющий вопрос. Не сокращай многочастную реплику до только понятной части,
если неясный хвост может содержать вопрос, просьбу или профессиональный вывод.

13. Совпадение общего итога не доказывает корректность аналитики.
Акт сверки, ОСВ, контрольная сумма или общий итог могут сходиться, даже если внутри
неверно сопоставлены договоры, документы расчетов, авансы и задолженность,
подразделения, заказы, партии или другие аналитические разрезы. Не подтверждай вывод
«все правильно» только потому, что сошелся общий итог. Сначала установи, совпадает
ли внутренняя аналитическая структура и должны ли встречные суммы действительно
зачитываться друг с другом. Для 60.01/60.02 отдельно проверь контрагента, договор,
документ расчетов, конкретное поступление/оплату и экономическую связь долга с
выданным авансом.
""".strip()
...
def _aggregate_match_claim_context(text: str) -> bool:
    """High-confidence trigger for 'total matches, therefore everything is correct'."""
    q = _norm(text)
    total_match = any(
        marker in q
        for marker in (
    ...
    return total_match and conclusion
...
def detect_professional_consistency_hints(text: str) -> list[str]:
    ...
    if _aggregate_match_claim_context(text):
        detail_60 = "60.01" in q and "60.02" in q
        hint = (
            "В условии совпадение общего итога/сверки используется как доказательство, "
            "что учет внутри корректен. Это недостаточно: совпадение общей суммы "
            "подтверждает только агрегированный итог и не доказывает правильность "
            "внутренней аналитики. Проверь разрезы, по которым суммы должны быть "
            "экономически сопоставлены, прежде чем соглашаться с выводом «все нормально»."
        )
        if detail_60:
            hint += (
                " Для 60.01 и 60.02 отдельно сопоставь контрагента, договор, документ "
                "расчетов, поступление, оплату, задолженность и выданный аванс: общий "
                "итог может сходиться при незачтенном или неверно адресованном авансе."
            )
        hints.append(hint)
